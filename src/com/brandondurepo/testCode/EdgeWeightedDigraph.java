...
/**
    ...
    public class EdgeWeightedDigraph {
        ...
        public EdgeWeightedDigraph(int V, int E) {
            ...
            for (int i = 0; i < E; i++) {
                int v = Math.abs(rand.nextInt() % V);
                int w = Math.abs(rand.nextInt() % V);
    ...
    public EdgeWeightedDigraph(Object[] labels, BigDecimal[][] weight){
            if(weight.length * weight[0].length != labels.length * labels.length)
                throw new IllegalArgumentException("A weight must be given for every edge.");
            ...
            for (int v = 0; v < V; v++)
                adj[v] = new Bag<DirectedEdge>();
            Iterator<Object> slowI = Arrays.asList(labels).iterator();
            Iterator<Object> fastI;

            while (slowI.hasNext()){
                Object latestVertex = slowI.next();
                fastI = Arrays.asList(labels).iterator();

                while (!fastI.next().equals(latestVertex)) {}

                Object temp;
                while (fastI.hasNext()) {
                    temp = fastI.next();

                    DirectedEdge e = new DirectedEdge((Integer) latestVertex , (Integer) temp, weight[(Integer) latestVertex][(Integer) temp].doubleValue());
                    DirectedEdge d = new DirectedEdge((Integer) temp , (Integer) latestVertex, weight[(Integer) temp][(Integer) latestVertex].doubleValue());
                    addEdge(e);
                    addEdge(d);
                }
            }
